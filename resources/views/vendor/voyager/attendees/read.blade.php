    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="panel panel-bordered" style="padding-bottom:5px;">
          <!-- form start -->
{{--          {{dd($dataTypeContent->presenterUrl)}}--}}
          @foreach($dataType->readRows as $row)
            ...
            @if (isset($row->details->view))
              ...
              @elseif($row->type == 'file')
                ...
                @endif
              @elseif($row->type == 'code_editor')
                {{--{{dd($dataTypeContent->{$row->field})}}--}}
                @forelse($dataTypeContent->{$row->field} as $mkey => $mod)
                  @foreach($mod as $key => $value)
                    <p>{{$key}}: {{$value['description']}} @if($mkey == 'payment') ${{$value['value']/100}}@endif</p>
                  @endforeach
                @empty
                <p>No modifiers</p>
                @endforelse
